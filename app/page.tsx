...
export default function Home() {
  ...
  // Subtle parallax: the fixed background grid + glow drift as you scroll, for
  // depth. Passive listener + rAF + GPU transforms only; off for reduced motion.
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        if (glowRef.current) glowRef.current.style.transform = `translate3d(0, ${y * 0.28}px, 0)`;
        raf = 0;
      });
    };
  ...
  return (
      <main className="relative flex min-h-screen flex-col items-center bg-gradient-to-br from-mirage-950 via-mirage-900 to-mirage-950">
        ...
        <div ref={gridRef} className="fixed inset-0 opacity-10 pointer-events-none will-change-transform" aria-hidden="true">
        ...
        <div ref={glowRef} className="fixed inset-0 overflow-hidden pointer-events-none opacity-20 will-change-transform" aria-hidden="true">
        ...
        <div className="relative z-10 w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-16">
          ...
          {/* Daily Challenge + 1v1 banners — side-by-side, or 1v1 alone
              when the daily is completed/missing. */}
          <HomeHeroBanners />
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
              ...
              {puzzles.map((puzzle, index) => (
                  <Reveal key={puzzle.href} delay={(index % 4) * 70} className="flex h-full">
          ...
          <Reveal as="div" className="grid md:grid-cols-3 gap-6">
          ...
          <Reveal as="div" className="bg-gradient-to-br from-mirage-900/80 to-mirage-950/80 rounded-2xl shadow-xl p-8 sm:p-10 border-2 border-spring-green-500/30 text-center hover:border-spring-green-400/60 transition-colors duration-300">
