  ...
  try {
    ...
    switch (type) {
      case 'level':
        ...
        leaderboard = await db.collection('users')
          .find({})
      ...
      case 'streak':
        if (userId) {
          ...
          return NextResponse.json({
            entry: {
              ...
              displayName: user.displayName,
              avatar: user.avatar,
              value: user.currentDailyStreak || 0,
              secondaryValue: user.longestDailyStreak || 0,
              verified: user.verified || false,
            }
          });
        }
        ...
        const streakTotalCount = await db.collection('users').countDocuments({});
        
        leaderboard = await db.collection('users')
          .find({})
          .sort({ currentDailyStreak: -1, longestDailyStreak: -1 })
          .skip(skip)
          .limit(limit)
          .toArray();
      ...
      case 'word-memory':
        ...
        if (userId) {
          ...
          if (isTimeBased) {
            ...
            rank = await db.collection('gameStats').countDocuments({
              game: type,
              isLeaderboardEligible: true,
        ...
        const gameStats = await db.collection('gameStats')
          .find({ 
            game: type,
            isLeaderboardEligible: true
          })
