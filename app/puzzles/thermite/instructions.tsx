        ...
        <>
            ...
            <InstructionSection id="howtoplay" title="How to Play - Step by Step" icon={<Gamepad className="w-5 h-5" />}>
                <ol className="list-decimal pl-5 space-y-2">
                    <li><strong>The board populates</strong> — a grid of pieces with every square highlighted, so your first click can go anywhere</li>
                    <li><strong>Click a highlighted square</strong> — its status drops one step (full → half → empty)</li>
                    <li><strong>Watch the new highlights propagate</strong> — the squares exactly the clicked piece&apos;s range away, along rows, columns, and diagonals, light up as your next legal moves</li>
                    <li><strong>Earn a kill</strong> — taking a square from half to empty scores +1 point (a &quot;byte&quot;)</li>
                    <li><strong>Chain combos</strong> — a fast streak of three kills whose ranges are all the same, or form a short→medium→long or long→medium→short run, flashes &quot;CRC Bypassed!&quot; and awards a bonus</li>
                    ...
                    <li><strong>Win at the target score</strong> — reach the target (24 for Sewer, 28 for Vault, 30 for Art Asylum) and the hack succeeds</li>
                    <li><strong>Lose if you stall</strong> — if a click highlights zero new squares, you&apos;re out of moves and the hack fails instantly. The timer running out fails it too</li>
                </ol>
            </InstructionSection>
            ...
            <InstructionSection id="scoring" title="Understanding the Scoring System" icon={<Trophy className="w-5 h-5" />}>
                ...
                <ul className="list-disc pl-5 space-y-2">
                    ...
                    <li><strong>Combo bonus</strong> — a streak of three kills, each landed within one second of the previous, whose piece ranges are all identical (e.g. short-short-short) or form an ascending/descending run (short-medium-long / long-medium-short), awards <code>2^N</code> points, where N is the number of combos you&apos;ve already chained in this run</li>
                    ...
                    <li><strong>Full → half clicks award nothing</strong> — they&apos;re setup moves to position the next kill, and they also reset your combo streak</li>
                    <li><strong>Target score</strong> — the win condition. Sewer preset = 24, Vault preset = 28, Art Asylum = 30</li>
                </ul>
                <p className="mt-3">
            ...
            <InstructionSection id="tips" title="Advanced Tips & Strategies" icon={<Lightbulb className="w-5 h-5" />}>
                ...
                <ul className="list-disc pl-5 space-y-2">
                    <li><strong>Know the piece patterns</strong> — every piece highlights up to 8 squares at exactly its range: short reaches the immediate neighbors, medium reaches cells exactly two away, long reaches exactly three away — always along rows, columns, and diagonals. Each range leaves a different set of follow-up options</li>
                    <li><strong>Pre-stage your kills</strong> — click full squares (no score) to leave a cluster of half squares ready to be killed in a fast burst. Finish the setup first: a full-square click mid-burst resets the streak</li>
                    <li><strong>Combos require speed</strong> — each kill must land within a second of the last, so you need to know your next two clicks before you start. Plan the sequence, then execute</li>
                    <li><strong>Watch the ranges you kill</strong> — the combo pattern is read off the killed squares&apos; pieces. A wrong-range kill doesn&apos;t break your streak; keep chaining fast kills and the bonus fires as soon as the last three complete a pattern</li>
                    <li><strong>Long-range pieces are gold</strong> — they reach three cells across the board and give you the most flexibility for the next click. Save them for when you&apos;re in a corner</li>
            ...
            <InstructionSection id="difficulty" title="Difficulty Progression Breakdown" icon={<TrendingUp className="w-5 h-5" />}>
                <p>
                    The three presets correspond to the in-game heists, but you can also configure custom difficulty in settings:
                </p>
                <ul className="list-disc pl-5 space-y-2">
                    <li><strong>Maze Bank - Sewer</strong> — 6×6 grid, target score 24, 60-second timer. The standard Maze Bank sewer hack</li>
                    <li><strong>Maze Bank - Vault</strong> — 6×6 grid, target score 28, 60-second timer. Same board size as Sewer but a higher score requirement means you have to chain combos to win in time</li>
                    <li><strong>Art Asylum - Water</strong> — 6×6 grid, target score 30, 25-second timer. The real skill check: less than half the time for the highest target</li>
                    <li><strong>Custom</strong> — open settings to change the grid (5×5 up to 9×9), target score (10–75), and timer (10–180 seconds). Useful for drilling combo timing on smaller boards or stress-testing your reads on larger ones</li>
                </ul>
                <p>
            ...
            <InstructionSection id="mistakes" title="Common Mistakes to Avoid" icon={<AlertTriangle className="w-5 h-5" />}>
                <ul className="list-disc pl-5 space-y-2">
                    ...
                    <li><strong>Trying to combo without setup</strong> — you need three half squares you can chain in quick succession, with ranges that complete a pattern (three matching, or a 1-2-3 / 3-2-1 run). If you don&apos;t pre-stage half squares, you&apos;ll never hit the timing</li>
                    <li><strong>Breaking your own streak</strong> — clicking a full square resets the combo counter, so don&apos;t mix setup clicks into your kill burst</li>
            ...
            <InstructionSection id="nopixel" title="NoPixel Roleplay Context" icon={<Briefcase className="w-5 h-5" />}>
                <p>
                    In NoPixel 4.0, the Thermite minigame is triggered when burning through security hardware during major heists. The three presets here mirror the actual in-game configurations:
                </p>
                <ul className="list-disc pl-5 space-y-2">
                    <li><strong>Maze Bank sewer</strong> — the entry hack for the sewer infiltration route. Lower target score, faster to complete</li>
                    <li><strong>Maze Bank vault</strong> — the hack inside the vault itself. Higher target score, you need combos to make the timer</li>
                    <li><strong>Art Asylum water</strong> — the hardest configuration: 30 points in just 25 seconds</li>
                </ul>
                <p>
            ...
            <InstructionSection id="training" title="Training Recommendations" icon={<GraduationCap className="w-5 h-5" />} defaultOpen={false}>
                ...
                <ul className="list-disc pl-5 space-y-2">
                    ...
                    <li><strong>Finish on Art Asylum</strong> — 30 points in 25 seconds only works with a combo-heavy run. If you can clear it consistently, nothing in-game will surprise you</li>
                    <li><strong>Shrink the grid</strong> — practice on a smaller custom board (5×5) to drill combo timing in a constrained space</li>
            ...
            <InstructionSection id="memory" title="Reading the Board" icon={<Brain className="w-5 h-5" />} defaultOpen={false}>
                ...
                <ul className="list-disc pl-5 space-y-2">
                    <li><strong>Range silhouettes</strong> — learn the shape of each piece&apos;s reach: a ring of up to 8 cells at exactly one, two, or three squares out. Glance at any piece and immediately see its kill zone</li>
                    <li><strong>Half-square clusters</strong> — scan the board for three half squares you can chain back-to-back whose ranges match or form a run. That&apos;s a combo waiting to happen</li>
            ...
            <InstructionSection id="why" title="Why This Matters" icon={<Target className="w-5 h-5" />} defaultOpen={false}>
                <p>
                    Thermite is the gatekeeper to the most valuable heists in NoPixel — the Maze Bank runs that elite vault crews build their scores around. Players who can consistently clear the Vault and Art Asylum presets under timer pressure get recruited for those crews and pull the big scores. Practice this mechanic until the chain reads are automatic, and you&apos;ll find yourself trusted with hacks the rest of the server can&apos;t reliably finish.
                </p>
            </InstructionSection>
        </>
    );
