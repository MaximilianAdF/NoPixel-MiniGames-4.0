...
// Memoized letter cell to avoid re-renders when its props don't change
const LetterCell: React.FC<LetterCellProps> = React.memo(({ letter, isActive, state }) => {
...
}, (prev, next) => prev.letter === next.letter && prev.isActive === next.isActive && prev.state === next.state);
...
const Chopping: FC = () => {
    ...
    const checkStatus = (stateBoardSnapshot: LetterState[]) => {
        /* If the final state is 'done', it can be concluded that all other letters are also 'done' */
        if (stateBoardSnapshot[stateBoardSnapshot.length - 1] === 'done') {
        ...
        if (stateBoardSnapshot[activeIndex] === 'fail') {
                                    ...
                                    <div key={rowIndex} className='game-grid-row' style={{ gridTemplateColumns: `repeat(${Math.min(numLetters - rowIndex * defaultGridCols, defaultGridCols)}, min-content)` }}>
                                        ...
                                        {Array.from({ length: defaultGridCols }).map((_, colIndex) => {
                                            ...
                                            if (letterIndex < numLetters) {
                                                ...
                                                const state = stateBoard[letterIndex];

                                                return (
                                                    <LetterCell key={letterIndex} letter={letter} isActive={isActive} state={state} />
                                                );
                                            }
...
// Give the memoized component a stable displayName for eslint
LetterCell.displayName = 'LetterCell';
