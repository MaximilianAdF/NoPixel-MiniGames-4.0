...
- **Snapshot** — at most 4-6 key numbers (traffic trend from Cloudflare/GSC — the trustworthy sources; top channel; one notable change). When `journey_ads` has data, ALWAYS include the **ad fill-rate trend** (impressions/day ÷ real pageviews/day, computed per the journey_ads note) and whether the ramp is on-track vs its gates. A small table or short list.
...
- **⚠️ Health flags** — only real issues (404 spikes, 5xx, **top Sentry errors**, bot/threat surges, broken pages, GA4 gaps).
- **🔭 Trends & opportunities** — verified market/news intel that matters (NoPixel 5.0 signals, trending games/features worth building, competitor moves), each with its source and the concrete action. Omit when nothing verified.
